...
export function reverseGCode(
    ...
    const toolPaths = [...toolPathsOrg.filter(x=>x.commandCodeType === "G")]
    ...
    for (let i = currentLine; i >= 1; i--) {
        ...
        if(curentGCodeCommand.hasMove)
        {
            let reversedGCodeCode = curentGCodeCommand.commandCode;

            if(curentGCodeCommand.commandCode == "G28" || curentGCodeCommand.commandCode == "G30") {reversedGCodeCode = "G0"}
            ...
            reversedCode.push(curentGCodeCommand.rawCommand)
